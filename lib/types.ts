...
export interface Account { id:string; name:string; kind:AccountKind; creditLimit?:number; balance?:number; currency:string; createdAt:string; sortOrder?:number; }
export interface Tx { id:string; type:TxType; amount:number; description:string; category:string; date:string; notes?:string; currency:string; createdAt:string; myShare?:number; accountId?:string; isTransfer?:boolean; }
